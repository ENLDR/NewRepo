...
IMAGE_FOLDER = 'local_pose_images'
...
def save_single_pose(pose_name, pose_result, r_id, image_data, frame_index):
    try:
        ...
        image_filename = f"{r_id}frame{frame_index + 1}.jpg"
...
def get_single_pose_details():
    try:
        # Get the Single_pose_id from the request
        single_pose_id = request.json.get('single_pose_id')
        if not single_pose_id:
            return jsonify({'error': 'Single_pose_id is required'}), 400

        db = get_db()  # Get database connection
        cursor = db.cursor()

        # Query to fetch details of the single pose and its associated correct pose
        query = '''
            SELECT 
            ...
            WHERE sp.Single_pose_id = %s
        '''
        cursor.execute(query, (single_pose_id,))
        result = cursor.fetchone()

        if not result:
            return jsonify({'error': 'Single pose details not found'}), 404

        # Prepare response with single pose image link
        single_pose_image_path = os.path.join(IMAGE_FOLDER, f'{single_pose_id}.jpg')
        if not os.path.exists(single_pose_image_path):
            return jsonify({'error': 'Single pose image not found'}), 404

        # Construct the response
        response = {
            "Single_pose_name": result[0],
            "Single_pose_result": float(result[1]),
            "Correct_Pose_name": result[2],
            "Pose_Details": result[3],
            "Correct_Pose_Img_link": result[4],
            "Single_Pose_Img_link": f"http://10.95.147.34:5000/local_pose_images/{single_pose_id}.jpg"
        }
